...
describe("resolveWinixAuthState", () => {
  ...
  it("refreshes legacy auth without an ID token even if its access token is fresh", async () => {
    const stored = buildAuth({ idToken: null, accessExpiresAt: 10_000 });
    const provider: WinixAuthProvider = {
      login: vi.fn(),
      refresh: vi.fn().mockResolvedValue(buildAuth({ idToken: "new-id-token" })),
    };

    const auth = await resolveWinixAuthState("u@example.com", "password", stored, 1_000, provider);

    expect(auth.idToken).toBe("new-id-token");
    expect(provider.refresh).toHaveBeenCalledWith(stored.refreshToken, stored.userId);
    expect(provider.login).not.toHaveBeenCalled();
  });

  it("replaces tokens from the retired Cognito client with a full login", async () => {
    const provider: WinixAuthProvider = {
      login: vi.fn().mockResolvedValue(buildAuth({ idToken: "new-id-token", refreshToken: "new-refresh" })),
      refresh: vi.fn().mockRejectedValue(new Error("Refresh token has expired")),
    };

    const auth = await resolveWinixAuthState(
      "u@example.com", "password", buildAuth({ idToken: null }), 1_000, provider,
    );

    expect(auth.refreshToken).toBe("new-refresh");
    expect(auth.idToken).toBe("new-id-token");
    expect(provider.login).toHaveBeenCalledWith("u@example.com", "password");
  });
